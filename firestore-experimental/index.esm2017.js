...
class IndexedDbPersistence {
    constructor(allowTabSynchronization, persistenceKey, clientId, platform, lruParams, queue, serializer, sequenceNumberSyncer, force) {
        this.allowTabSynchronization = allowTabSynchronization;
        this.persistenceKey = persistenceKey;
        this.clientId = clientId;
        this.queue = queue;
        ...
        if (platform.window && platform.window.localStorage) {
            ...
            if (force === false) {
                error(LOG_TAG$1, 'LocalStorage is unavailable. As a result, persistence may not work ' +
    ...
    static async createIndexedDbPersistence(options) {
        ...
        const persistence = new IndexedDbPersistence(options.allowTabSynchronization, options.persistenceKey, options.clientId, options.platform, options.lruParams, options.queue, options.serializer, options.sequenceNumberSyncer, options.force);
     ...
     * primary lease.
     */
    updateClientMetadataAndTryBecomePrimary(force = false) {
...
class IndexedDbPersistenceSettings {
    constructor(cacheSizeBytes, synchronizeTabs, force) {
    ...
    startIndexedDbPersistence(user, settings) {
        ...
        return Promise.resolve().then(async () => {
            if (settings.synchronizeTabs &&
                ...
                throw new FirestoreError(Code.UNIMPLEMENTED, 'IndexedDB persistence with synchronizeTabs is only available on ' +
...
class Firestore {
    ...
    enablePersistence(settings) {
        ...
        return this.configureClient(new IndexedDbPersistenceSettings(this._settings.cacheSizeBytes, synchronizeTabs, experimentalForce));
    }
    clearPersistence() {
